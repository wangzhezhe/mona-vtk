...
// since the drc is started by the colza server
  ...
  while (step < g_num_iterations)
  {
    ...
    if (leader)
    {
      spdlog::info("iteration {} nblocks_per_proc {} compute time is {} and {} ", step,
    ...
    if (leader)
    {
      spdlog::info("wait time for iteration {}: {} ", step, waitEend - waitSart);
    }
    ...
    if (leader)
    {
      spdlog::debug("iteration {} stage time {}", step, stageEnd - stageStart);
    }
    // start the execute
